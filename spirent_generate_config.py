...
if __name__ == '__main__':
    ...
    config = StcConfig()
    config.data[StcIPv4.config_key]['protocol'] = args.protocol
    config.data[StcIPv4.config_key]['destAddr'] = args.dstaddr
    config.data[StcIPv4.config_key]['sourceAddr'] = args.srcaddr
    # hardcoded for ilab. A.B.C.D --> A.B.C.1
    config.data[StcIPv4.config_key]['gateway'] = '.'.join(args.srcaddr.split('.')[:3]+['1'])
